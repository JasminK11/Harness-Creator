...
| M9 | `catalog/intents.yaml` mit den zwölf Absichten + `harness.mjs intent <id>` | Der Absichts-Schnitt, den Miraje fordert, ohne 1.091 Bausteine neu zu klassifizieren. Ersetzt die hartkodierte Symptomtabelle in `harness-build`. **Teilvermerk (2026-08-10):** CLI-Teil (`intent`-Subcommand) und `catalog/intents.yaml` umgesetzt und adversarial geprüft (Beleg und Verlauf: `knowledge/LOG.md`, Einträge vom 2026-08-10 zum Subcommand `intent`); der Skill-Teil — die Symptomtabelle in `harness-build/SKILL.md` durch `intent` zu ersetzen — steht noch aus | 1,5 Tage | CLI + neue Datei + Skill |
