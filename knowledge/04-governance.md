...
Der Ansatz existierte im Keim bereits. `harness-build/SKILL.md` führte eine Symptomtabelle („Reviews übersehen dieselben Fehler" → `search "code review" --type agent`; „Keiner traut sich an den Code" → `search "codebase onboarding"`; „Deployments schlagen fehl" → `search "deployment ci" --domain devops`). Das war eine Absichts-Ebene mit sechs Einträgen, hartkodiert in einer Skill-Datei, ohne Verbindung zum Katalog. Der Vorschlag war, sie herauszuziehen und aufzuwerten — `catalog/intents.yaml`, von Hand gepflegt, überlebt jedes `extract`:

**Umgesetzt (2026-08-10):** Die Symptomtabelle ist ersetzt. `harness-build/SKILL.md` Schritt 2 nutzt jetzt `catalog/intents.yaml` und den Subcommand `intent <id>` (unten beschrieben); siehe Zeile M9 in der Tabelle „Sollten wir tun" und `knowledge/LOG.md`.
...
Die `suche`-Strings dürfen `--type`- und `--domain`-Flags eingebettet enthalten, z. B. `"code review --type agent"` oder `"Vertragsrecht --domain legal-de"` (so in der inzwischen angelegten `catalog/intents.yaml`). `parseSucheQuery()` trennt das Flag vom Suchtext und wendet es als Filter nur auf diese eine Query an — dieselbe Semantik wie die gleichnamigen `search`-Flags, einschliesslich der Eigenschaft, dass ein eingebettetes `--domain` eines Massen-Repos dieses Repo für die Query öffnet (Beleg: `search "code review" --type agent` liefert 32 Treffer, exakt das Mass, auf das `intent pruefen` die Query `code review --type agent` einengt). Ein unbekanntes `--`-Token in einem `suche`-String gilt als Datenfehler in der Datei und wird mit Warnung ignoriert statt als Suchwort durchgereicht — sonst verfälschte es die UND-Suche und liesse die Query fälschlich auf null Treffer laufen.
...
Der Audit ist genau der Eval aus Abschnitt 4, plus eine Bestandshygiene-Prüfung, die `extract` nebenbei erhebt und in `CHANGELOG.md` schreibt (M11, s. Abschnitt 6 — seit 2026-08-10 umgesetzt und adversarial geprüft):
...
Vier Kennzahlen, bei jedem `/harness-update` erhoben, als Zeitreihe im `CHANGELOG.md` — Bestand, kein Vorschlag mehr. Wenn „ohne brauchbare Description" nach einem Update springt, hat ein Repo etwas getan, das wir sehen müssen. Das ist Coherence ohne Bürokratie — Miraje: „Governance muss keine Bürokratie sein, es hängt an der Automatisierung."
...
| M9 | ~~`catalog/intents.yaml` mit den zwölf Absichten + `harness.mjs intent <id>`~~ — **erledigt** | Der Absichts-Schnitt, den Miraje fordert, ohne 1.091 Bausteine neu zu klassifizieren. Ersetzte die hartkodierte Symptomtabelle in `harness-build`. **Erledigt (2026-08-10):** CLI-Teil (`intent`-Subcommand, `catalog/intents.yaml`) und Skill-Teil (`harness-build/SKILL.md` Schritt 2 auf den `intent`-Zugang umgestellt) sind beide umgesetzt und adversarial geprüft. Der Skill-Umbau wurde in der ersten Fassung abgelehnt und nach vier Korrekturauflagen freigegeben: die Anker-Zahl war fälschlich pauschal mit „drei" angegeben (`testen` und `ausliefern` tragen nur zwei), eine Deckungsbehauptung zu „Architektur" war ungeprüft, und der Sprachhinweis-Satz unterschlug, dass er nur bei deutsch aussehenden Anfragen feuert. Verlauf und Belege: `knowledge/LOG.md`, Einträge vom 2026-08-10 zum Subcommand `intent` und zum Skill-Umbau samt Prüfauflage | erledigt | CLI + neue Datei + Skill |
...
| M11 | ~~Katalog-Hygiene-Kennzahlen bei jedem `extract` erheben und in `CHANGELOG.md` schreiben~~ — **erledigt** | Macht Coherence-Verfall sichtbar, bevor jemand ihn spürt. Vier Zahlen, keine Bürokratie. **Erledigt (2026-08-10):** `katalogHygiene(catalog)` läuft bei jedem `extract` (und darüber bei `update`) und schreibt vier Kennzahlen als Zeitreihe ins `CHANGELOG.md`. Nenner bewusst Standardzugriff + Quarantäne, ohne Massen-Repos (1.099) — nicht `cmdStats()`s engerer Standardzugriff (1.091): würde Quarantäne den Nenner vorher verlassen, wäre die Description-Kennzahl strukturell für immer 0, weil genau `quarantaeneGrund()` beide Mengen trennt. Adversarial geprüft und mit vier erfüllten Auflagen angenommen. Verlauf und Belege: `knowledge/LOG.md`, Eintrag „Katalog-Hygiene-Kennzahlen bei jedem `extract`" (2026-08-10) | erledigt | CLI |

Empfohlene Reihenfolge: **M1, M2, M3, M6, M7, M9, M10 und M11 sind erledigt** (M2 und M3 am 2026-08-10, siehe Nachtrag in 3.2; M9 am 2026-08-10, CLI- und Skill-Teil, siehe Zeile M9 oben; M11 am 2026-08-10, siehe Zeile M11 oben), und M4, M5 und M8 sind in `knowledge/06` unter anderen Nummern aufgegangen und dort umgesetzt (Prüfregeln und Auswahlkriterien in `harness-build/SKILL.md`, `evals/routing.jsonl` samt `eval`-Subcommand, Bestätigung und Sichtprüfung vor dem Kopieren). Damit ist jede Massnahme dieser Tabelle erledigt oder anderweitig in `knowledge/06` aufgegangen — offen bleibt nur, was unter „Wäre theoretisch schön" und „Ausdrücklich nicht" steht.
...
| LLM-gestützte Neuklassifikation aller Bausteine nach Absicht | Der saubere Miraje-Schnitt. Kosten: ein Modelllauf über 1.104 Bausteine bei jedem Update, plus derselbe Lauf über 24.543 im Massen-Repo, wenn man es je öffnet. M9 liefert 80 % davon für 5 % der Kosten |
